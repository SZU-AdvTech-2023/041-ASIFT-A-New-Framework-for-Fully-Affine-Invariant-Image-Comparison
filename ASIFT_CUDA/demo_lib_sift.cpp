...
//		octSize *= 0.5;
	...
	//printf("gaussian convolution use %lf seconds.\n",conv_duration);
	...
	while (image.nwidth() > minsize && image.nheight() > minsize && OctaveCounter < par.OctaveMax) {
		...
		OctaveKeypoints(image, octSize, keypoints, par);//每层的特征点
	...
	//printf("OctaveKeypoints finally use % lf seconds.\n", OctaveKeypoints_duration_total);
	//printf("sample finally use %lf seconds.\n", sample_duration_total);
...
void OctaveKeypoints(flimage& image, float octSize, keypointslist& keys, siftPar& par)
{
	...
	*/
	...
	//printf("OK_combine use %lf seconds.\n", (double)(end - start) / CLOCKS_PER_SEC);
	...
	start = clock();
	FindMaxMin(dogs, blur, octSize, keys, par);//寻找特征点 计算特征描述子
	end = clock();

	//printf("FindMaxMin use %lf seconds.\n", (double)(end - start) / CLOCKS_PER_SEC);
	...
	float octSize, keypointslist& keys, siftPar& par)
{
	...
	*/
			...
			for (int c = par.BorderDist; c < width - par.BorderDist; c++) {
				...
				*/
				...
				if (fabs(val) > 0.8 * par.PeakThresh)
				{
					...
					*/
					if (LocalMaxMin(val, dogs[s - 1], r, c) && LocalMaxMin(val, dogs[s], r, c) && LocalMaxMin(val, dogs[s + 1], r, c) && NotOnEdge(dogs[s], r, c, octSize, par))//空间极值
					{
						partialcounter++;
						...
						InterpKeyPoint(
							dogs, s, r, c, grad, ori,
							map, octSize, keys, 5, par);//特征描述子
						end = clock();

						kp_total += (double)(end - start) / CLOCKS_PER_SEC;
						//getchar();
					}
	...
	//printf("compute gradient orientation finally use %lf seconds.\n", total);
	//printf("InterpKeyPoint finally use %lf seconds.\n", kp_total);
}
	...
	float octSize, keypointslist& keys, int movesRemain, siftPar& par)
{
	...
	*/
	...
	AssignOriHist(
		...
		r + offset[1], c + offset[2], keys, par);//特征描述子
	//else
	//	AssignOriAvg(
	//		grad, ori, octSize, octScale,
...
//	float	hist[par.OriBins], distsq, dif, gval, weight, angle, interp;
	...
	for (int i = 0; i < par.OriBins; i++) {
		...
		if (hist[i] > hist[prev] && hist[i] > hist[next] &&
			...
			MakeKeypoint(
				grad, ori, octSize, octScale,
				octRow, octCol, angle, keys, par);//特征描述子
		}
	...
	float octRow, float octCol, float angle, keypointslist& keys, siftPar& par)
{
	...
	MakeKeypointSample(newkeypoint, grad, ori, octScale, octRow, octCol, par);//特征描述子
