...
#define H2 (SCALEDOWN_H/2)
    ...
    const int xl = fminf(width - 1, fmaxf(0, xp - 2));
    const int yl = fminf(height - 1, fmaxf(0, yp - 2));
...
#define dx2 (SCALEDOWN_W/2)
    ...
    int maxtx = fminf(dx2, width / 2 - xStart / 2);
...
__global__ void ScaleUp_gpu(float* d_Result, float* d_Data, int width, int pitch, int height, int newpitch)
{
    ...
    if (x < 2 * width && y < 2 * height) {
        ...
        int xr = fminf(xl + 1, width - 1);
        int yd = fminf(yu + 1, height - 1);
...
__global__ void ExtractSiftDescriptors_gpu(cudaTextureObject_t texObj, SiftPoint* d_sift, int fstPts, float subsampling)
{
    ...
    tsum1 = fminf(buffer[idx] * rsqrtf(tsum1), 0.2f);
...
__device__ float FastAtan2_gpu(float y, float x)
{
    float absx = abs(x);
    float absy = abs(y);
    float a = __fdiv_rn(fminf(absx, absy), fmaxf(absx, absy));
    float s = a * a;
...
__global__ void ExtractSiftDescriptorsCONSTNew_gpu(cudaTextureObject_t texObj, SiftPoint* d_sift, float subsampling, int octave, int d_MaxNumPoints, unsigned int* d_PointCounter)
{
    ...
    int fstPts = fminf((int)d_PointCounter[2 * octave - 1], d_MaxNumPoints);
    int totPts = fminf((int)d_PointCounter[2 * octave + 1], d_MaxNumPoints);
    //if (tx==0 && ty==0)
    ...
    for (int bx = blockIdx.x + fstPts; bx < totPts; bx += gridDim.x) {
        ...
        for (int y = ty; y < 16; y += 8) {
            ...
            float angf = 4.0f / 3.1415f * FastAtan2_gpu(dy, dx) + 4.0f;
...
#define LEN 32                                   //%%%% Note: Lowe suggests 36, not 32
    ...
    int fstPts = (fminf)((int)d_PointCounter[2 * octave - 1], d_MaxNumPoints);
    int totPts = (fminf)((int)d_PointCounter[2 * octave + 0], d_MaxNumPoints);
    for (int bx = blockIdx.x + fstPts; bx < totPts; bx += gridDim.x) {
        ...
        for (int i = tx; i < WID * WID; i += blockDim.x) {
            int y = i / WID;
            int x = i - y * WID;
            int xp = (fmaxf)((fminf)(x - RAD + xi, w - 1), 0);
            int yp = (fmaxf)((fminf)(y - RAD + yi, h - 1), 0);
...
__global__ void OrientAndExtractCONST_gpu(cudaTextureObject_t texObj, SiftPoint* d_Sift, float subsampling, int octave, int d_MaxNumPoints, unsigned int* d_PointCounter)
{
    ...
    for (int bx = blockIdx.x + fstPts; bx < totPts; bx += gridDim.x) {
        ...
        __syncthreads();
        ExtractSiftDescriptor_gpu(texObj, d_Sift, subsampling, octave, bx); //%%%%
        if (idx < d_MaxNumPoints) //%%%%
            ExtractSiftDescriptor_gpu(texObj, d_Sift, subsampling, octave, idx); //%%%%
    }
}
...
#define MEMWID (MINMAX_W + 2)
    ...
    int maxx = (fminf)(minx + MINMAX_W, width);
    int xpos = minx + tx;
    int size = pitch * height;
    int ptr = size * scale + (fmaxf)((fminf)(xpos - 1, width - 1), 0);
    ...
    if (Shuffle(maxv, 0) > thresh) {
        ...
        int yptr0 = ptr + (fmaxf)(0, ypos - 1) * pitch;
        int yptr2 = ptr + (fminf)(height - 1, ypos + 1) * pitch;
...
#define MEMWID (MINMAX_W + 2)
    ...
    int yloops = (fminf)((int)(height - MINMAX_H * blockIdx.y), MINMAX_H);
    float maxv = 0.0f;
...
__global__ void LaplaceMultiMem_gpu(float* d_Image, float* d_Result, int width, int pitch, int height, int octave, float* d_LaplaceKernel)
{
    ...
    const int xp = blockIdx.x * LAPLACE_W + tx;
    const int yp = blockIdx.y;
    float* data = d_Image + (int)fmaxf(fminf(xp - LAPLACE_R, width - 1), 0);
    float temp[2 * LAPLACE_R + 1], kern[LAPLACE_S][LAPLACE_R + 1];
    if (xp < (width + 2 * LAPLACE_R)) {
        for (int i = 0; i <= 2 * LAPLACE_R; i++)
            temp[i] = data[(int)fmaxf(0, fminf(yp + i - LAPLACE_R, height - 1)) * pitch];
...
__global__ void LaplaceMultiMemWide_gpu(float* d_Image, float* d_Result, int width, int pitch, int height, int octave, float* d_LaplaceKernel)
{
    ...
    float* data = d_Image + (int)fmaxf((fminf)(xp - 4, width - 1), 0);
    float temp[9];
    if (xp < (width + 2 * LAPLACE_R)) {
        for (int i = 0; i < 4; i++)
            temp[i] = data[(int)fmaxf(0, (fminf)(yp + i - 4, height - 1)) * pitch];
        for (int i = 4; i < 8 + 1; i++)
            temp[i] = data[(int)fminf(yp + i - 4, height - 1) * pitch];
        for (int scale = 0; scale < LAPLACE_S; scale++) {
            float* kernel = d_LaplaceKernel + octave * 12 * 16 + scale * 16;
            for (int i = 0; i <= LAPLACE_R; i++)
...
__global__ void LaplaceMultiMemTest_gpu(float* d_Image, float* d_Result, int width, int pitch, int height, int octave, float* d_LaplaceKernel)
{
    ...
    for (int i = 0; i < 4; i++)
        temp[i] = data[(int)fmaxf(0, (fminf)(yp + i - 4, h)) * pitch];
    for (int i = 4; i < 8 + LAPLACE_H; i++)
        temp[i] = data[(int)fminf(yp + i - 4, h) * pitch];
    for (int i = 0; i <= LAPLACE_R; i++)
...
__global__ void LaplaceMultiMemOld_gpu(float* d_Image, float* d_Result, int width, int pitch, int height, int octave, float* d_LaplaceKernel)
{
    ...
    float* data = d_Image + (int)fmaxf((fminf)(xp - 4, width - 1), 0);
    int h = height - 1;
    sdata1[tx] = kernel[0] * data[(int)fminf(yp, h) * pitch] +
        kernel[1] * (data[(int)fmaxf(0, (int)fminf(yp - 1, h)) * pitch] + data[(int)fminf(yp + 1, h) * pitch]) +
        kernel[2] * (data[(int)fmaxf(0, (int)fminf(yp - 2, h)) * pitch] + data[(int)fminf(yp + 2, h) * pitch]) +
        kernel[3] * (data[(int)fmaxf(0, (int)fminf(yp - 3, h)) * pitch] + data[(int)fminf(yp + 3, h) * pitch]) +
        kernel[4] * (data[(int)fmaxf(0, (int)fminf(yp - 4, h)) * pitch] + data[(int)fminf(yp + 4, h) * pitch]);
    __syncthreads();
    float* sdata2 = data2 + LAPLACE_W * scale;
    if (tx < LAPLACE_W) {
        sdata2[tx] = kernel[0] * sdata1[tx + 4] +
...
__global__ void LowPass_gpu(float* d_Image, float* d_Result, int width, int pitch, int height, float* d_LowPassKernel)
{
    ...
    int h = height - 1;
    if (yp < height)
        buff[tx] = kernel[4] * data[(int)fminf(yp, h) * pitch] +
        kernel[3] * (data[(int)fmaxf(0, fminf(yp - 1, h)) * pitch] + data[(int)fminf(yp + 1, h) * pitch]) +
        kernel[2] * (data[(int)fmaxf(0, fminf(yp - 2, h)) * pitch] + data[(int)fminf(yp + 2, h) * pitch]) +
        kernel[1] * (data[(int)fmaxf(0, fminf(yp - 3, h)) * pitch] + data[(int)fminf(yp + 3, h) * pitch]) +
        kernel[0] * (data[(int)fmaxf(0, fminf(yp - 4, h)) * pitch] + data[(int)fminf(yp + 4, h) * pitch]);
    __syncthreads();
...
__global__ void LowPassBlockOld_gpu(float* d_Image, float* d_Result, int width, int pitch, int height, float* d_LowPassKernel)
{
    ...
    int xl = fmaxf(fminf(xp - 4, width - 1), 0);
    for (int l = -8; l <= LOWPASS_H; l += 4) {
        if (l < LOWPASS_H) {
            int yl = fmaxf(fminf(yp + l + 4, height - 1), 0);
