...
{
    int d_MaxNumPoints=siftData.maxPts;
    float* d_LaplaceKernel;
    cudaMalloc((void**)&d_LaplaceKernel, (8 * 12 * 16) * sizeof(float));
    unsigned int* d_PointCounter;
    cudaMalloc((void**)&d_PointCounter, (8 * 2 + 1) * sizeof(int));

    TimerGPU timer(0);
    //unsigned int* d_PointCounterAddr;
    //safeCall(cudaGetSymbolAddress((void**)&d_PointCounterAddr, d_PointCounter));
    safeCall(cudaMemset((void*)d_PointCounter, 0, (8 * 2 + 1) * sizeof(int)));
    //safeCall(cudaMemcpy(d_MaxNumPoints, &siftData.maxPts, sizeof(int),cudaMemcpyHostToDevice));
    ...
    if (!scaleUp) {
        ...
        safeCall(cudaMemcpy(d_LaplaceKernel, kernel, 8 * 12 * 16 * sizeof(float),cudaMemcpyHostToDevice));
        LowPass(lowImg, img, (std::max)(initBlur, (double)0.001));
        TimerGPU timer1(0);
        ExtractSiftLoop(siftData, lowImg, numOctaves, 0.0f, thresh, lowestScale, 1.0f, memoryTmp, memorySub + height * iAlignUp(width, 128), d_MaxNumPoints, d_LaplaceKernel, d_PointCounter);
        safeCall(cudaMemcpy(&siftData.numPts, (d_PointCounter + 2 * numOctaves), sizeof(int), cudaMemcpyDeviceToHost));
        ...
        TimerGPU timer1(0);
        ScaleUp(upImg, img);
        LowPass(lowImg, upImg, (std::max)(initBlur, (double)0.001));
        float kernel[8 * 12 * 16];
        PrepareLaplaceKernels(numOctaves, 0.0f, kernel);
        safeCall(cudaMemcpy(d_LaplaceKernel, kernel, 8 * 12 * 16 * sizeof(float),cudaMemcpyHostToDevice));
        ExtractSiftLoop(siftData, lowImg, numOctaves, 0.0f, thresh, lowestScale * 2.0f, 1.0f, memoryTmp, memorySub + height * iAlignUp(width, 128), d_MaxNumPoints, d_LaplaceKernel, d_PointCounter);
        safeCall(cudaMemcpy(&siftData.numPts, (d_PointCounter + 2 * numOctaves), sizeof(int), cudaMemcpyDeviceToHost));
        siftData.numPts = (siftData.numPts < siftData.maxPts ? siftData.numPts : siftData.maxPts);
...
#endif
    cudaFree(d_LaplaceKernel);
    cudaFree(d_PointCounter);
...
int ExtractSiftLoop(SiftData& siftData, CudaImage& img, int numOctaves, double initBlur, float thresh, float lowestScale, float subsampling, float* memoryTmp, float* memorySub, int d_MaxNumPoints, float* d_LaplaceKernel, unsigned int* d_PointCounter)
{
    ...
    if (numOctaves > 1) {
        ...
        ExtractSiftLoop(siftData, subImg, numOctaves - 1, totInitBlur, thresh, lowestScale, subsampling * 2.0f, memoryTmp, memorySub + (h / 2) * p, d_MaxNumPoints, d_LaplaceKernel, d_PointCounter);
    }
    ExtractSiftOctave(siftData, img, numOctaves, thresh, lowestScale, subsampling, memoryTmp, d_MaxNumPoints, d_LaplaceKernel, d_PointCounter);
#ifdef VERBOSE
    double totTime = timer.read();
...
void ExtractSiftOctave(SiftData& siftData, CudaImage& img, int octave, float thresh, float lowestScale, float subsampling, float* memoryTmp, int d_MaxNumPoints, float* d_LaplaceKernel, unsigned int* d_PointCounter)
{
    ...
    LaplaceMulti(texObj, img, diffImg, octave, d_LaplaceKernel);
    FindPointsMulti(diffImg, siftData, thresh, 10.0f, 1.0f / NUM_SCALES, lowestScale / subsampling, subsampling, octave, d_MaxNumPoints, d_PointCounter);
#ifdef VERBOSE
    double gpuTimeDoG = timer1.read();
    TimerGPU timer4;
#endif
    ComputeOrientations(texObj, img, siftData, octave, d_MaxNumPoints, d_PointCounter);
    ExtractSiftDescriptors(texObj, siftData, subsampling, octave, d_MaxNumPoints ,d_PointCounter);
...
double ScaleDown(CudaImage& res, CudaImage& src, float variance)
{
    ...
    float* d_ScaleDownKernel;
    cudaMalloc((void**)&d_ScaleDownKernel, 5 * sizeof(float));
    ...
    if (oldVariance != variance) {
        ...
        safeCall(cudaMemcpy(d_ScaleDownKernel, h_Kernel, 5 * sizeof(float),cudaMemcpyHostToDevice));
    ...
    ScaleDown_gpu << <blocks, threads >> > (res.d_data, src.d_data, src.width, src.pitch, src.height, res.pitch, d_ScaleDownKernel);
#endif
    checkMsg("ScaleDown() execution failed\n");
    cudaFree(d_ScaleDownKernel);
    return 0.0;
}
...
double ScaleUp(CudaImage& res, CudaImage& src)
{
    ...
    ScaleUp_gpu << <blocks, threads >> > (res.d_data, src.d_data, src.width, src.pitch, src.height, res.pitch);
...
#if 1
    dim3 threads(11 * 11);
    ComputeOrientationsCONST_gpu << <blocks, threads >> > (texObj, siftData.d_data, octave, d_MaxNumPoints, d_PointCounter);
#else
    dim3 threads(256);
...
#else
    ExtractSiftDescriptorsCONSTNew_gpu << <blocks, threads >> > (texObj, siftData.d_data, subsampling, octave, d_MaxNumPoints, d_PointCounter);
#endif
...
#else
    OrientAndExtractCONST_gpu << <blocks, threads >> > (texObj, siftData.d_data, subsampling, octave, d_MaxNumPoints, d_PointCounter);
#endif
...
double RescalePositions(SiftData& siftData, float scale)
{
    ...
    dim3 threads(64);
    RescalePositions_gpu << <blocks, threads >> > (siftData.d_data, siftData.numPts, scale);
...
{
    float* d_LowPassKernel;
    cudaMalloc((void**)&d_LowPassKernel, (2 * LOWPASS_R + 1) * sizeof(float));
    ...
    if (scale != oldScale) {
        ...
        safeCall(cudaMemcpy(d_LowPassKernel, kernel, (2 * LOWPASS_R + 1) * sizeof(float),cudaMemcpyHostToDevice));
        oldScale = scale;
    }
    int width = res.width;
    int pitch = res.pitch;
    ...
    LowPassBlock_gpu << <blocks, threads >> > (src.d_data, res.d_data, width, pitch, height, d_LowPassKernel);
#else
...
double LaplaceMulti(cudaTextureObject_t texObj, CudaImage& baseImage, CudaImage* results, int octave, float* d_LaplaceKernel)
{
    ...
    LaplaceMultiMem_gpu << <blocks, threads >> > (baseImage.d_data, results[0].d_data, width, pitch, height, octave, d_LaplaceKernel);
#endif
#if 0
    dim3 threads(LAPLACE_W + 2 * LAPLACE_R, LAPLACE_S);
    dim3 blocks(iDivUp(width, LAPLACE_W), iDivUp(height, LAPLACE_H));
    LaplaceMultiMemTest_gpu << <blocks, threads >> > (baseImage.d_data, results[0].d_data, width, pitch, height, octave, d_LaplaceKernel);
#endif
#if 0
    dim3 threads(LAPLACE_W + 2 * LAPLACE_R, LAPLACE_S);
    dim3 blocks(iDivUp(width, LAPLACE_W), height);
    LaplaceMultiMemOld_gpu << <blocks, threads >> > (baseImage.d_data, results[0].d_data, width, pitch, height, octave, d_LaplaceKernel);
#endif
#if 0
    dim3 threads(LAPLACE_W + 2 * LAPLACE_R, LAPLACE_S);
    dim3 blocks(iDivUp(width, LAPLACE_W), height);
    LaplaceMultiTex_gpu << <blocks, threads >> > (texObj, results[0].d_data, width, pitch, height, octave, d_LaplaceKernel);
#endif
...
#else
    FindPointsMultiNew_gpu << <blocks, threads >> > (sources->d_data, siftData.d_data, w, p, h, subsampling, lowestScale, thresh, factor, edgeLimit, octave, d_MaxNumPoints, d_PointCounter);
#endif
#endif
