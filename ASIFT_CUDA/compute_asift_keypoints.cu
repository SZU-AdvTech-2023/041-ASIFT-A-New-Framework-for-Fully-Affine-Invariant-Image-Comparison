...
float* getKernel(int& ksize, float sigma)
{
	float x, sum = 0;
...
void getMem()
{
	...
	printf("Device Memory available: %d\n", avail / 1024 / 1024);
	printf("Device Memory totally: %d\n\n", total / 1024 / 1024);
}


/* -------------- MAIN FUNCTION ---------------------- */

int compute_asift_keypoints_npp_CudaSift(vector<float>& image, int width, int height, int num_of_tilts, int verb, vector<vector< keypointslist > >& keys_all, int sstep)
{
	...
	int dstep = 0;
	float* image_d = nppiMalloc_32f_C1(width, height, &dstep);
	cudaError_t wtferr = cudaMemcpy2D(image_d, dstep, image.data(), width*sizeof(float), width * sizeof(float), height, cudaMemcpyHostToDevice);
	//cudaMalloc((float**)&image_d, width * height * sizeof(float));
	//cudaMemcpy(image_d, image.data(), width * height * sizeof(float), cudaMemcpyHostToDevice);
	...
	for (tt = 1; tt <= num_tilt; tt++)
	{
		...
		if (t == 1)
		{
			...
			CudaImage image_cur;
			image_cur.Allocate(width, height, iAlignUp(width, 128), false, NULL, image_tmp1_float);
			image_cur.Download();
			...
			ExtractSift(siftdata, image_cur, 5, 1.0f, 3.0f, 0.0f, false, memoryTmp);

			for (int i = 0; i < siftdata.numPts; i++)
			{
				keypoint tmp;
				tmp.x = siftdata.h_data[i].xpos;
				tmp.y = siftdata.h_data[i].ypos;
				tmp.scale = siftdata.h_data[i].scale;
				tmp.angle = siftdata.h_data[i].orientation;
				memcpy(tmp.vec, siftdata.h_data[i].data, 128 * sizeof(float));
				keys_all[tt - 1][0].push_back(tmp);
			}

			//printf("Detect %d sift keypoints.\n\n", keys_all[tt - 1][0].size());
		...
		else
		{
			...
			for (rr = 1; rr <= num_rot1; rr++)
			{
				...
				sROI.x = 0;
				sROI.y = 0;
				NppiSize sSize{ width,height };
				double abBox[2][2];
				NppStatus rbstt = nppiGetRotateBound(sROI, abBox, theta, 0, 0);

				NppiRect dROI;
				dROI.height = ceil(fabs(abBox[1][1] - abBox[0][1]));
				dROI.width = ceil(fabs(abBox[1][0] - abBox[0][0]));
				dROI.x = 0;
				dROI.y = 0;
				int rstep = 0;
				Npp32f* image_1 = nppiMalloc_32f_C1(dROI.width, dROI.height, &rstep);

				NppStatus rstt = nppiRotate_32f_C1R(image_d, sSize, dstep, sROI, image_1, rstep, dROI, theta, -abBox[0][0], -abBox[0][1], NPPI_INTER_NN);
				end = clock();
				double duration = (double)(end - start) / CLOCKS_PER_SEC;
				//cout << "nppiRotate " << duration << endl << endl;
				//float* image_1 = NULL;

				//image_1 = frot_gpu(image_d, image_1, width, height, &width_r, &height_r, theta, frot_b, frot_k);

				/* Tilt */
				int width_t = (int)(dROI.width * t1);
				int height_t = (int)(dROI.height * t2);
				...
				int ksize = 0;
				float* kernel = getKernel(ksize, sigma_aa);

				float* kernel_d;
				cudaMalloc((float**)&kernel_d, ksize * sizeof(float));
				cudaMemcpy(kernel_d, kernel, ksize * sizeof(float), cudaMemcpyHostToDevice);

				int gstep = 0;
				Npp32f* image_g = nppiMalloc_32f_C1(dROI.width, dROI.height, &gstep);
				NppStatus gstt = nppiFilterColumn_32f_C1R(image_1, rstep, image_g, gstep, NppiSize{ dROI.width,dROI.height }, kernel_d, ksize, ksize / 2);
				free(kernel);
				cudaFree(kernel_d);
				end = clock();
				duration = (double)(end - start) / CLOCKS_PER_SEC;
				//cout << "nppiFilterColumn " << duration << endl << endl;
				//GaussianBlur1D_gpu(image_1, width_r, height_r, sigma_aa, flag_dir);

				// simulate a tilt: subsample the image along the vertical axis by a factor of t.
				start = clock();
				double tQuad[4][2] = { {0,0},{width_t,0},{width_t,height_t},{0,height_t} };
				double coeffs[2][3];
				NppStatus gcstt = nppiGetAffineTransform(dROI, tQuad, coeffs);

				int tstep = 0;
				Npp32f* image_2 = nppiMalloc_32f_C1(width_t, height_t, &tstep);
				NppiRect tROI = dROI;
				tROI.width = width_t;
				tROI.height = height_t;
				NppStatus tstt = nppiWarpAffine_32f_C1R(image_g, NppiSize{ dROI.width,dROI.height }, gstep, dROI, image_2, tstep, tROI, coeffs, NPPI_INTER_NN);
				end = clock();
				duration = (double)(end - start) / CLOCKS_PER_SEC;
				//cout << "nppiWarpAffine " << duration << endl << endl;
				//image_2 = fproj_gpu(image_1, image_2, width_r, height_r, &fproj_sx, &fproj_sy, fproj_bg, fproj_o, fproj_p, &fproj_i, fproj_x1, fproj_y1, fproj_x2, fproj_y2, fproj_x3, fproj_y3, fproj_x4, fproj_y4);
				cudaFree(image_1);
				cudaFree(image_g);
				if (verb)
				{
				...
				/*
				float* image_final;
				int pitch = iAlignUp(width_t, 128);
				cudaMallocPitch((void**)&image_final, (size_t*)&pitch, (size_t)(sizeof(float) * width_t), (size_t)height_t);
				pitch /= sizeof(float);
				cudaMemcpy2D(image_final, pitch * sizeof(float), image_2, sizeof(float) * width_t, sizeof(float) * width_t, height_t, cudaMemcpyDeviceToDevice);
				cudaFree(image_2);
				*/
				CudaImage image_cur;
				image_cur.Allocate(width_t, height_t, tstep / sizeof(float), false, image_2, NULL);
				//image_cur.Download();

				SiftData siftdata;
				InitSiftData(siftdata, 32768, true, true);

				float* memoryTmp = AllocSiftTempMemory(width_t, height_t, 5, false);
				ExtractSift(siftdata, image_cur, 5, 1.0f, 3.0f, 0.0f, false, memoryTmp);
				cudaFree(image_2);
				...
				duration = (double)(end - start) / CLOCKS_PER_SEC;
				//cout << "Totally use " << duration << " seconds." << endl;

				//printf("tt:%d, rr:%d, Detect %d sift keypoints.\n\n", tt, rr, keypoints_filtered.size());
...
int compute_asift_keypoints_rtg_openmp_CudaSift(vector<float>& image, int width, int height, int num_of_tilts, int verb, vector<vector< keypointslist > >& keys_all)
{
	vector<float> image_t, image_tmp1, image_tmp;
	...
	for (tt = 1; tt <= num_tilt; tt++)
	{
		...
		else
		{
			...
			for (rr = 1; rr <= num_rot1; rr++)
			{
				...
				CudaImage image_cur;
				image_cur.Allocate(width_t, height_t, pitch, false, image_final, NULL);
				image_cur.Download();
...
int compute_asift_keypoints_rtg_CudaSift(vector<float>& image, int width, int height, int num_of_tilts, int verb, vector<vector< keypointslist > >& keys_all)
{
	vector<float> image_t, image_tmp1, image_tmp;
...
int compute_asift_keypoints_openmp_CudaSift(vector<float>& image, int width, int height, int num_of_tilts, int verb, vector<vector< keypointslist > >& keys_all)
{
	vector<float> image_t, image_tmp1, image_tmp;
		...
		else
		{
			...
			for (rr = 1; rr <= num_rot1; rr++)
			{
				...
				*/
				...
				double duration = (double)(end - start) / CLOCKS_PER_SEC;
				*/
				//if(verb)printf("Sift use %lf seconds.\n", duration);
				CudaImage image_cur;
				image_cur.Allocate(width_t, height_t, iAlignUp(width_t, 128), false, NULL, image_tmp.data());
				image_cur.Download();
...
int compute_asift_keypoints_CudaSift(vector<float>& image, int width, int height, int num_of_tilts, int verb, vector<vector< keypointslist > >& keys_all)
{
	vector<float> image_t, image_tmp1, image_tmp;
...
int compute_asift_keypoints_openmp(vector<float>& image, int width, int height, int num_of_tilts, int verb, vector<vector< keypointslist > >& keys_all)
{
	vector<float> image_t, image_tmp1, image_tmp;
...
int compute_asift_keypoints(vector<float>& image, int width, int height, int num_of_tilts, int verb, vector<vector< keypointslist > >& keys_all)
	...
	for (tt = 1; tt <= num_tilt; tt++)
	{
		...
		else
		{
			...
			for (rr = 1; rr <= num_rot1; rr++)
			{
				...
				float* image_tmp1_float = new float[width_t * height_t];
				for (int cc = 0; cc < width_t * height_t; cc++)
					image_tmp1_float[cc] = image_tmp[cc];
